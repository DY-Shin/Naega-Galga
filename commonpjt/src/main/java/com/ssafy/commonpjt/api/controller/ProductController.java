...
//        }
    ...
    @PostMapping("/wish")
    public ResponseEntity<Object> addWishList(@RequestBody Map<String, Integer> addData) throws Exception {
        int statusCode = productService.addWishList((int) addData.get("userIndex"), (int) addData.get("productIndex"));
        return ResponseEntity.status(statusCode).body(null);
    }

//    @DeleteMapping("/wish/{wishListIndex}")
//    public ResponseEntity<Object> deleteWishList(@PathVariable int wishListIndex) throws Exception {
//        int statusCode = productService.deleteWishList(wishListIndex);
//        return ResponseEntity.status(statusCode).body(null);
//    }
}
